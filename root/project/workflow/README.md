...
1. **Consult the Obsidian vault** — Resolve the vault root by running `bin/workflow-index vault` (**not** by reading `config.json` directly; see [Configuration](#configuration)), then search the user's Obsidian knowledge vault for notes related to the topic (prior reasoning, domain concepts, references the user has curated). This is a *search* step ("any related prior thinking?"), not a context dump — surface relevant pointers during dialogue and capture useful ones in the thought file's `## References` section. Skip for trivial decisions, mechanical refactors, or topics clearly out-of-scope for the vault. The trigger is *engagement*, not source directory: a scrap item that escalates from one-line triage into real sparring gets the same treatment as a fresh thought.

   **If the task fails, say so in the dialogue before continuing** — one line naming the error, e.g. *"heads-up: the vault isn't reachable (`vault_root` points at a directory that doesn't exist), so this spar has no prior-notes check."* Never let an unreachable vault look like a vault with nothing in it. That silence — a stale path or an absent config producing "no related notes found" — is exactly how this step got skipped unnoticed before; the task exists to make it loud, but only surfacing it here makes the user able to act on it.
...
6. **If promoted** — Re-consult the vault with implementation-shaping queries in mind (constraints, prior decisions, related domain notes) before writing the backlog file. Capture findings inline in the plan or under `## References`. Write the `## Acceptance criteria` section (and `## Out of scope` if relevant) into the backlog file. Then perform the move (see "Moving files between stages" below).
